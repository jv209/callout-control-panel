/**
 * Callout CSS manager — generates CSS rules for custom callout types and
 * optionally writes them to a vault snippet file.
 * Source: obsidian-admonition v10.3.2 (MIT, Jeremy Valentine)
 *
 * NOTE: This file references a plugin interface. When adapting, replace the
 * plugin type import with your own plugin class.
 */

import { Component, MarkdownPostProcessorContext, Notice, setIcon } from "obsidian";
...
export default class CalloutManager extends Component {
    ...
    generateCssString() {
        const sheet = [
            `/* This snippet was auto-generated by the Callout Manager plugin */\n\n`
        ];
    ...
    ${color}
    --callout-icon: ${admonition.icon.name};
}`;
        } else {
            rule = `.callout[data-callout="${admonition.type.toLowerCase()}"] {
       ${color}
