/**
 * Input validation for callout type creation/editing/import.
 * Source: obsidian-admonition v10.3.2 (MIT, Jeremy Valentine)
 *
 * NOTE: Methods take a plugin reference for checking duplicate types and
 * resolving icons. When adapting to your plugin, replace the plugin type
 * with your own plugin class or extract an interface.
 */

import { Admonition, AdmonitionIconDefinition } from "../@types";
import { t } from "../lang/helpers";

/** Plugin interface — the minimum surface area needed by the validator. */
export interface ValidatorPluginRef {
    data: { userAdmonitions: Record<string, Admonition> };
    iconManager: {
        getIconType(name: string): string | undefined;
