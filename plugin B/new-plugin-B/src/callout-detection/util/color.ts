/**
 * Color utilities for parsing and converting CSS colors.
 *
 * Extracted from: obsidian-callout-manager/src/util/color.ts
 ...
 * Does not support non-RGB color formats (e.g. HSV).
 */
...
export function parseColorRGB(rgb: string): RGB | null {
	...
	if (rgbComponents === null) return null;

	if (undefined !== rgbComponents.find((v) => isNaN(v) || v < 0 || v > 0xff)) return null;

	return { r: rgbComponents[0], g: rgbComponents[1], b: rgbComponents[2] };
}
...
export function parseColorRGBA(rgba: string): RGBA | null {
	...
	if (undefined !== allComponents.find((v) => isNaN(v) || v < 0 || v > 0xff)) return null;

	return { r: allComponents[0], g: allComponents[1], b: allComponents[2], a: allComponents[3] };
}
...
export function parseColorHex(hex: string): RGB | RGBA | null {
	...
	if (undefined !== hexComponents.find((v) => isNaN(v) || v < 0 || v > 0xff)) return null;

	const hexRGB: RGB | RGBA = { r: hexComponents[0], g: hexComponents[1], b: hexComponents[2] };
...
function rgbComponentStringsToNumber(components: [string, string, string]): [number, number, number] | null {
	if (components[0].endsWith('%')) {
		if (undefined !== components.slice(1, 3).find((c) => !c.endsWith('%'))) return null;
		return components
